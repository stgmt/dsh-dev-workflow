...
- **Хот-релоад скилов — НЕ гарантирован в живой сессии.** Каталог реестра кэшируется и обновляется только по событию инвалидации (ватчер/fs-наблюдение корней). Глобальный корень `~/.dsh/skills`, созданный ПОСЛЕ старта сессии (или правки в нём), может не подхватиться до рестарта — проверяй в новой сессии. Проверено вживую: `skill(name)` вернул «unknown» для скила, файл которого появился после старта сессии.
